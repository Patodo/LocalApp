...
test("the repository exposes only the localapp npm product", async () => {
  ...
  assert.equal(rootManifest.scripts["test:localapp-package"], "node --test --test-concurrency=1 packages/localapp/scripts/pack-package.node-test.mjs packages/localapp/scripts/build-package.node-test.mjs scripts/single-package-cutover.node-test.mjs");
