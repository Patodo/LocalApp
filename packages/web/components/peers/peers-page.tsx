...
type SyncJob = {
  id: string; ownerId: string; appName: string; peerId: string; syncId: string; withData: false;
...
const CANCELLABLE = new Set(["queued", "staging", "validating", "backing-up", "installing"]);
...
export function PeersPage() {
  ...
  const startSync = async (peer: Peer) => {
    ...
    try {
      const job = await requestJson<SyncJob>(`/api/me/apps/${encodeURIComponent(appName)}/sync`, {
        ...
        body: JSON.stringify({ peerId: peer.id, withData: false }),
      });
