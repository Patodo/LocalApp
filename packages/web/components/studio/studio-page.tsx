...
export function StudioPage() {
  ...
  return <div className="space-y-8">
    ...
    <section><h2 className="mb-3 text-lg font-semibold">我的工作区</h2>{workspaces.length === 0 ? <p className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">暂无工作区</p> : <div className="space-y-2">{workspaces.map((workspace) => <div key={workspace.id} className="flex items-center justify-between rounded-lg border bg-card p-4"><div><p className="font-medium">{workspace.name}</p><p className="text-xs text-muted-foreground">更新于 {new Date(workspace.updatedAt).toLocaleString()}</p></div><Button variant="outline" size="sm" onClick={() => setSelected(workspace)}><FilePenLine className="mr-1 size-4" />编辑 {workspace.name}</Button></div>)}</div>}</section>
    {selected && <section className="space-y-3 rounded-xl border bg-card p-4"><h2 className="text-lg font-semibold">编辑 {selected.name}</h2><div className="space-y-2"><Label htmlFor="file-path">文件路径</Label><Input id="file-path" value={filePath} onChange={(event) => setFilePath(event.target.value)} placeholder="src/app.tsx" /></div><div className="space-y-2"><Label htmlFor="file-content">文件内容</Label><Textarea id="file-content" value={content} onChange={(event) => setContent(event.target.value)} rows={12} /></div><Button onClick={() => void saveFile()} disabled={!filePath.trim()}>保存文件</Button></section>}
  </div>;
}
