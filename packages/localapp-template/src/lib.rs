...
const RUNTIME_COMPAT_MARKER: &str = ".localapp-runtime-compat";
...
pub fn extract_cli_zone(target_dir: &Path) -> Result<(), String> {
    ...
    expose_runtime_compatibility_path(target_dir)?;
    Ok(())
}

/// 删除由 `extract_cli_zone` 创建的根目录 runtime 兼容视图。
///
/// 新架构的真实 CLI 领地是 `.localapp/runtime/`。模板测试和旧项目仍会
/// 读取 `runtime/`，所以这里优先创建相对符号链接；不支持符号链接的平台
/// 使用带标记的副本。只有能确认是我们创建的视图时才会删除，避免触碰用户目录。
pub fn remove_runtime_compatibility_path(target_dir: &Path) -> Result<(), String> {
    let compatibility = target_dir.join("runtime");
    let metadata = match fs::symlink_metadata(&compatibility) {
        Ok(metadata) => metadata,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(()),
        Err(error) => {
            return Err(format!(
                "Failed to inspect runtime compatibility path {}: {error}",
                compatibility.display()
            ));
        }
    };
    let is_symlink = metadata.file_type().is_symlink();
    let is_managed_copy = metadata.is_dir() && compatibility.join(RUNTIME_COMPAT_MARKER).is_file();
    ...
    if is_symlink {
        ...
        fs::remove_dir_all(&compatibility)
    }
    .map_err(|error| {
        format!(
            "Failed to remove runtime compatibility path {}: {error}",
            compatibility.display()
        )
    })
}

fn expose_runtime_compatibility_path(target_dir: &Path) -> Result<(), String> {
    let source = target_dir.join(".localapp/runtime");
    if !source.is_dir() {
        return Ok(());
    }
    remove_runtime_compatibility_path(target_dir)?;
    let compatibility = target_dir.join("runtime");

    #[cfg(unix)]
    let link_result = std::os::unix::fs::symlink(Path::new(".localapp/runtime"), &compatibility);
    #[cfg(windows)]
    let link_result =
        std::os::windows::fs::symlink_dir(Path::new(".localapp/runtime"), &compatibility);
    #[cfg(not(any(unix, windows)))]
    let link_result: Result<(), std::io::Error> = Err(std::io::Error::other(
        "runtime compatibility links are unsupported on this platform",
    ));
    ...
    copy_directory(&source, &compatibility)?;
    fs::write(
        compatibility.join(RUNTIME_COMPAT_MARKER),
        b"managed by localapp\n",
    )
    .map_err(|error| {
        format!(
            "Failed to mark runtime compatibility copy {}: {error}",
            compatibility.display()
        )
    })?;
    Ok(())
...
/// 判断脚本字符串是否调用了 `localapp dev`（避免 dev:vite 递归引用 localapp dev）。
///
/// 从 CLI scripts 模块下沉而来，供 package.json 后处理使用。
...
mod tests {
    ...
    #[test]
    fn extract_cli_zone_exposes_runtime_compatibility_path_for_template_tests() {
        let tmp = tempfile::tempdir().unwrap();
        let target = tmp.path().join("test-project");
        fs::create_dir_all(&target).unwrap();
        ...
        assert!(target.join("runtime/dev-shell.tsx").is_file());
        assert!(target.join("runtime/vite-plugin.mjs").is_file());
    }

    #[test]
    fn write_runtime_version_writes_given_version() {
        let tmp = tempfile::tempdir().unwrap();
        let target = tmp.path().join("test-project");
        fs::create_dir_all(&target).unwrap();

        write_runtime_version(&target, "9.9.9").unwrap();
        ...
        assert_eq!(parsed["cliVersion"].as_str(), Some("9.9.9"));
    }
  ...
  "peerDependencies": {
    ...
    #[test]
    fn script_invokes_localapp_dev_detects_dev_command() {
