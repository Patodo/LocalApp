...
describe("server run foreground ownership", () => {
  it("does not resolve supervisor exit until owned-tree termination completes", async () => {
    ...
    const artifact = path.join(fixture, "packed");
    ...
    let settled = false;
    let spawned = false;
    const result = runServerForeground({}, { artifactDirectory: artifact, spawnOwnedProcess: () => { spawned = true; return owned; } }).then((value) => { settled = true; return value; });
