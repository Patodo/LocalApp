...
### Requirement: 上传新页面（自动生成 pageId）
**Reason**: 不再使用随机 pageId，页面必须先通过 `POST /api/pages` 创建（带 name），再上传文件
**Migration**: 先调用 `localapp init` + `localapp new` 创建页面，再 `localapp upload` 上传文件

### Requirement: 版本管理端到端验证
**Reason**: 版本管理不在本变更范围内，版本相关测试保持不变
