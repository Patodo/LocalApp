...
- **WHEN** MinIO 连接失败（ECONNREFUSED 或超时），用户通过 `POST /api/content/upload` 上传文件
- **THEN** 文件写入本地 `data/{userId}/{pageName}/content/{nanoid}.{ext}`，返回 201 含 `{ key, url }` 格式与 S3 模式一致
