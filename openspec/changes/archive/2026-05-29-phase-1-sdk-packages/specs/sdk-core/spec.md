...
#### Scenario: 在应用 iframe 中自动检测 basePath
...
`@localapp/sdk` SHALL 导出 `redirectToLogin()` 函数。该函数 SHALL 将父窗口（如果存在）或当前窗口重定向到平台登录页，并携带 `redirect` 参数。

#### Scenario: 在 iframe 中调用登录重定向
- **WHEN** 页面运行在 iframe 中，调用 `redirectToLogin()`
- **THEN** 父窗口（`window.parent`）跳转到 `/login?redirect=<当前页面URL>`
