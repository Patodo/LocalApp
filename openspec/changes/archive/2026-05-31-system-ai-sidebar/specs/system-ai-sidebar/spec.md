...
系统级 AI 侧边栏组件，浮动覆盖在 iframe 应用之上，提供统一的 AI 对话界面。包含聊天消息列表、工具调用可视化、Composer 输入框。宽度可拖拽调整并通过浏览器 localStorage 持久化。

## ADDED Requirements

### Requirement: AI 侧边栏浮动面板

侧边栏 SHALL 以绝对定位浮动在 iframe 之上，不改变 iframe 的尺寸。侧边栏 SHALL 固定在容器右侧，从顶部延伸到底部。默认宽度 SHALL 为 380px。

#### Scenario: 侧边栏展开时不影响 iframe 布局
- **WHEN** 用户点击 AI 按钮展开侧边栏
- **THEN** iframe 保持原始宽度和高度不变
- **THEN** 侧边栏覆盖在 iframe 右侧区域之上
...
### Requirement: postMessage 工具执行桥接

侧边栏的 Agent SHALL 通过 postMessage 与 iframe 通信执行工具。当 Agent 产生 tool_call 时，SHALL 向 iframe 发送 `{ type: "localapp:tool_call", callId, toolName, args }`。iframe 返回结果后，SHALL 将结果传递给 Agent 继续。

#### Scenario: 工具调用发送到 iframe
- **WHEN** Agent 产生 tool_call（如 `fillForm`）
- **THEN** Shell 通过 postMessage 向 iframe 发送 `{ type: "localapp:tool_call", callId: "c1", toolName: "fillForm", args: { field: "name", value: "张三" } }`

#### Scenario: 接收工具结果
- **WHEN** iframe 返回 `{ type: "localapp:tool_result", callId: "c1", result: "已填写 name" }`
- **THEN** Agent 收到结果并继续对话

#### Scenario: 工具执行超时
- **WHEN** 工具调用在 30 秒内未收到 iframe 的结果响应
- **THEN** Agent 将该工具调用标记为错误，告知用户工具执行超时

### Requirement: 系统工具直接执行

系统工具（`getCurrentUser`、`queryData`、`listSchemas`）SHALL 在 Shell 层直接执行，不通过 postMessage 转发到 iframe。

#### Scenario: 系统工具在 Shell 层执行
- **WHEN** Agent 调用 `getCurrentUser` 工具
- **THEN** Shell 直接调用 `/api/me` API
- **THEN** 结果直接返回给 Agent，不涉及 iframe 通信
