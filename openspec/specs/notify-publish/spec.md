...
定义 notify 端点的发布 API：路径、payload 校验、跨 app 防护、权限三级模型（Level 1 owner-only / Level 2 系统约定表 / Level 3 自定义 SQL）、Rate limiting、通知持久化与响应格式。本规格覆盖 notify 端点的发布侧，订阅与投递侧见 notify-subscribe。
