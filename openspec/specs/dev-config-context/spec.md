...
TBD — 开发配置上下文规格，定义 CLI 开发命令写入 dev-config.json 的页面上下文信息和 Vite 代理路径改写行为。

## Requirements

### Requirement: CLI dev 命令写入 dev-config.json

`localapp dev` 命令 SHALL 在启动本地开发服务器前，向项目根目录的 `.localapp/dev-config.json` 写入页面上下文信息。文件 SHALL 包含 `serverUrl`、`userId`、`pageName`、`apiKey`、`miniServerPort` 字段。
...
- **THEN** CLI 启动本地 mini-server 并写入 `.localapp/dev-config.json`，内容为 `{ "serverUrl": "...", "userId": "...", "pageName": "...", "apiKey": "...", "miniServerPort": 5174 }`
- **THEN** 终端提示 "Dev config written to .localapp/dev-config.json"

#### Scenario: dev-config 包含 miniServerPort
- **WHEN** `localapp dev` 成功启动本地 mini-server
- **THEN** `.localapp/dev-config.json` SHALL 包含 `miniServerPort`
- **AND** `miniServerPort` SHALL 是 mini-server 实际监听端口

#### Scenario: 缺少 miniServerPort 时降级
- **WHEN** `.localapp/dev-config.json` 缺少 `miniServerPort`
- **THEN** DevShell SHALL 不能启用本地 dev API 工具
- **AND** Vite proxy SHALL 使用既有生产 server 转发规则保持向后兼容
...
### Requirement: Vite dev proxy 将 dev API 固定转发到 mini-server

Vite 开发服务器配置 SHALL 将 `/api/dev/*` 请求固定转发到 `.localapp/dev-config.json` 中 `miniServerPort` 指向的 mini-server。`/api/dev/*` SHALL NOT 转发到生产 `serverUrl`，也 SHALL NOT 注入生产 API Key。

#### Scenario: DevShell 请求 dev context
- **WHEN** DevShell 发起 `GET /api/dev/context`
- **AND** `dev-config.json` 包含 `miniServerPort`
- **THEN** Vite proxy SHALL 将请求转发到 `http://127.0.0.1:<miniServerPort>/api/dev/context`
- **AND** 响应来自本地 mini-server

#### Scenario: dev API 不注入生产鉴权
- **WHEN** `dev-config.json` 包含非空 `apiKey`
- **AND** DevShell 发起 `/api/dev/*` 请求
- **THEN** Vite proxy SHALL NOT 向该请求注入 `X-API-Key`
- **AND** 请求 SHALL NOT 离开本机开发环境
